...
/// JA: CTG council evaluation orchestration 用の provider vote output を保持します。
/// </summary>
public sealed class CtgCouncilEvaluationProviderRegistry
{
    ...
    /// JA: gate semantics を評価せず provider vote output を登録します。
    /// </summary>
    ...
    /// JA: 登録済み provider vote output を fail-closed result として解決します。
    /// </summary>
    ...
    /// JA: 登録済み provider vote output を決定論的な council 順で解決します。
    /// </summary>
    /// <param name="cancellationToken">EN: The cancellation token. JA: キャンセル通知を監視するトークンです。</param>
    ...
    /// JA: 登録済み provider vote output を消去します。
    /// </summary>
    public void Clear()
        => _outputs.Clear();
